...
if (-not (Test-Path -LiteralPath $cargoToml)) {
    throw "Could not find rust/Cargo.toml next to install-cliclaw.ps1."
}
...
$sourceBinary = Join-Path $targetDir "cliclaw.exe"
$destinationBinary = Join-Path $installDirFull "cliclaw.exe"
...
if (-not $SkipBuild) {
    ...
    $buildArgs = @("build", "--package", "rusty-claude-cli", "--bin", "cliclaw")
    if ($Profile -eq "release") {
        $buildArgs += "--release"
    }

    Write-Host ""
    Write-Host "Building cliclaw..."
...
Write-Host "  cliclaw"
Write-Host "  cliclaw prompt `"summarize this repository`""
Write-Host ""
Write-Host "Behavior:"
Write-Host "  - The folder where you run cliclaw becomes the active workspace."
Write-Host "  - cliclaw defaults to danger-full-access."
Write-Host "  - cliclaw allows broad working directories such as your home folder."

if ($pathWasUpdated) {
    Write-Host ""
    Write-Host "PATH was updated for your user account and the current PowerShell session."
    Write-Host "Open a new terminal if another shell still cannot find cliclaw."
} elseif ($NoPathUpdate) {
    Write-Host ""
